...
class TestLoadHoldings:
    ...
    def test_missing_file_returns_empty_list(self, tmp_path):
        # holdings.csv is absent — loader must return [] not raise FileNotFoundError
        d = tmp_path / "no_holdings"
        d.mkdir()
        loader = CSVLoader(d)
        result = loader.load_holdings()
        assert result == []

    def test_missing_file_does_not_raise(self, tmp_path):
        # Regression guard: ensure FileNotFoundError is never surfaced
        d = tmp_path / "no_holdings2"
        d.mkdir()
        loader = CSVLoader(d)
        try:
            loader.load_holdings()
        except FileNotFoundError:
            pytest.fail("load_holdings() raised FileNotFoundError when holdings.csv was absent")

    def test_returns_holdings_when_file_present(self, tmp_path):
        d = tmp_path / "with_holdings"
        d.mkdir()
        (d / "holdings.csv").write_text(HOLDINGS_CSV, encoding="utf-8")
        loader = CSVLoader(d)
        holdings = loader.load_holdings()
        assert len(holdings) == 2
